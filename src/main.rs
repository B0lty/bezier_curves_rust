...
fn main() {
    ...
    // Declaring buttons
    let arr_buttons: Vec<(f32, f32, String, f32)> = vec![
        (WIDTH as f32 - 107.0, 10.0, String::from("add point"), 3.0),
        (
            WIDTH as f32 - 107.0,
            30.0,
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        for button in &arr_buttons {
            ...
            for x in (-1 * button.3 as i32)
                ..((button.2.len() * font::GLYPH_WIDTH) as i32 + button.3 as i32)
            {
                for y in (-1 * button.3 as i32)..(font::GLYPH_HEIGHT as i32 + button.3 as i32) {
                        ...
                        && (y + text_y_offset as i32) < HEIGHT as i32
                    {
                        ...
                        buffer[index as usize] = button_bg_colour;
                    }
                }
            }
